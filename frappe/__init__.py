...
def has_permission(doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False):
	...
	if throw and not out:
		if doc:
			frappe.throw(_("No permission for {0}").format(doc.doctype + " " + doc.name),exc=frappe.PermissionError)
		else:
			frappe.throw(_("No permission for {0}").format(doctype),exc=frappe.PermissionError)
